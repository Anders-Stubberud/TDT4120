...
def find_clusters(E, n, k):
    ...
    def find_tree(member):
        for tree in arr:
            if member in tree.members:
                return tree

    while n > k:
        edge = E.pop(0)
        cluster1 = find_tree(edge[0])
        cluster2 = find_tree(edge[1])
        if cluster1 != cluster2:
            arr.remove(cluster1)
            arr.remove(cluster2)
            arr.append(Tree(cluster1.members + cluster2.members))
            n -= 1

    return [tree.members for tree in arr]
