...
def least_energy(reactions, start, goal, laws_of_thermodynamics=True):
    ...
    for reaction in reactions:
        u, v = reaction[0], reaction[1]
        mapper[u], mapper[v] = Node(u, math.inf), Node(v, math.inf)
    mapper[start].d = 0
    for _ in range(len(mapper) - 1):
        idle = True
        for reaction in reactions:
            u, v = reaction[0], reaction[1]
            curv = mapper[v].d
            mapper[v].d = relax(mapper[u].d, mapper[v].d, reaction[2])
            if not mapper[v].d == curv: 
                mapper[v].p = mapper[u]
                idle = False
        if idle : break
    ...
    def backtrack(u):
        ...
        backtrack(u.p)
        res.append(u.n)
    backtrack(mapper[goal])
    return res

def relax(ud, vd, w):
    return ud + w if ud + w < vd else vd
