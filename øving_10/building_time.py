...
def building_time(tasks):
    mapper = {}
    def backtrack(task, total):
        dependencies = task.depends_on
        if task in mapper : return mapper[task]
        if not dependencies:
            mapper[task] = task.time
            return task.time
        local = 0
        for dependency in dependencies : local = max(local, backtrack(dependency, total))
        mapper[task] = local + task.time
        return local + task.time
    res = 0
    for task in tasks : res = max(res, backtrack(task, 0))
    return res
