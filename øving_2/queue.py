...
class Queue:
    #se over forelesning for info om hvordan skille full/tom queue
    ...
    def enqueue(self, value):
        if self.head == self.tail and self.queue[self.head] != None: return
        self.queue[self.tail] = value
        self.tail = self.tail + 1 if self.tail < self.max_size - 1 else 0

    def dequeue(self):
        if self.head == self.tail and self.queue[self.head] == None: return
        res = self.queue[self.head]
        self.queue[self.head] = None
        self.head = self.head + 1 if self.head < self.max_size - 1 else 0
        return res
